        ...
        try {
            ...
            if (response.status === 200) {
                const tasks = data.map(obj => <div key={obj._id} className="grid grid-cols-3 gap-3 p-3 border-2 rounded-xl m-3">
                    <li>{obj.task}</li>
                    {obj.complete ? <button className="border-2 p-1 rounded-2xl bg-green-600 w-full h-10 hover:scale-105">Completed</button> : <button className="border-2 p-1 rounded-2xl bg-orange-600 w-full h-10 hover:scale-105" onClick={()=> {completeTask(obj.username,obj.task)}}>Mark Complete!!</button> }
